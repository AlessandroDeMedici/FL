...
from vae import Autoencoder, train, test, get_latent, get_parameters, set_parameters, predict, model_init
...
def client_fn(cid: str) -> FlowerClient:
    ...
    # Load model
    model = Autoencoder()
    model.load_state_dict(torch.load('data/vae.pth'))
    net = model.to(DEVICE)
...
if __name__ == "__main__":
    ...
    if args.classify is not None and args.classify:
        ...
        # Mapping each predicted cluster to the most common actual label
        cluster_mapping = {}

        for cluster in unique_clusters:
            # Get indices of points in this predicted cluster
            cluster_indices = np.where(y_predette == cluster)[0]
            
            # Get the actual labels for points in this cluster
            actual_labels_in_cluster = testloader.dataset[:][1][cluster_indices].numpy()
            
            # Find the most common actual label in this predicted cluster
            most_common_label = mode(actual_labels_in_cluster).mode
            
            # Map the predicted cluster to this most common actual label
            cluster_mapping[cluster] = most_common_label

        # Reassign clusters based on the most common actual label in each cluster
        y_predette_mapped = y_predette
        '''
    ...
    if args.tsne is not None and args.tsne:
        ...
        scatter = plt.scatter(data_tsne[:, 0], data_tsne[:, 1], c=testloader.dataset[:][1], cmap='cividis' ,s=50, alpha=0.7)
        plt.colorbar(scatter, label='Cluster')
        plt.title("Actual")
        plt.xlabel("x")
        plt.ylabel("y")
