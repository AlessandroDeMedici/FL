...
#out_dim = (input_dim - kernel + 2 * padding)/stride + 1
...
class Autoencoder(nn.Module):
    def __init__(self, z_dim = 16):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.LeakyReLU(True),
            nn.Flatten(0),
            nn.Linear(256 * 2 * 2, 512), # output 512
            nn.LeakyReLU(True),
            nn.Linear(512, z_dim), # output 16
            nn.Sigmoid()
        )
...
def train(model, train_loader, num_epochs, device):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00001) # lower learning rate
    model.train()
